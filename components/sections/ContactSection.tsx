...
function TypewriterText({ text, visible }: { text: string; visible?: boolean }) {
  ...
  useEffect(() => {
    // Penjelasan: Menghentikan re-render loop jika elemen tidak terlihat di layar. Ini sangat menghemat CPU/Baterai dan menghilangkan lag global di HP saat scroll.
...
export default function ContactSection({
  ...
  return (
    <section
      ...
      ref={ref}
      className={cn(
        "section-container border-t border-[--border] pt-10 pb-10",
        className,
      )}
    >
      <div className="mx-auto max-w-4xl text-center">
        <div className={`fade-up ${visible ? "visible" : ""}`}>
          ...
          <h2 className="font-display text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-bold tracking-tight text-[--text-primary] mb-6 md:mb-8">
            Get In <span className="gradient-text">Touch</span>
          </h2>

          <p className="mx-auto max-w-2xl text-sm sm:text-lg md:text-xl text-[--text-secondary] font-light leading-relaxed mb-8 md:mb-12">
        ...
        <div
          className={`mt-10 mx-auto max-w-3xl w-full flex flex-col items-center gap-6 fade-up delay-1 ${visible ? "visible" : ""}`}
        >
          {/* Top Divider Line */}
          ...
          <div className="relative w-full flex flex-col items-center py-2">
            {/* Soft Ambient Glow - Dikurangi blur-nya di HP agar lebih ringan */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-lg h-[120px] bg-[#25D366] opacity-10 blur-[40px] md:blur-[80px] pointer-events-none rounded-full" />
            ...
            <div className="relative z-10 w-full flex flex-col items-center gap-6 px-4 text-center">
              ...
              <div className="min-h-[120px] sm:min-h-[80px] md:min-h-[60px] flex items-center justify-center w-full max-w-2xl mx-auto">
                <p className="font-display text-[15px] sm:text-lg md:text-xl lg:text-2xl font-bold tracking-wider leading-loose md:leading-relaxed text-center text-[--accent-light]">
                  <TypewriterText text="Yang mau collabs project bisa hubungin WHATSAPP saya, Dan yang butuh jasa pembuatan website bisa juga hubungin saya." visible={visible} />
                </p>
              </div>

              {/* Native WhatsApp Button */}
              {/* Penjelasan: Mengganti transition-all dan menambah will-change-transform */}
              <a
                href="https://wa.me/6285119744035"
                target="_blank"
                rel="noopener noreferrer"
                className="group relative inline-flex h-14 md:h-16 items-center justify-center gap-3 overflow-hidden rounded-full bg-gradient-to-r from-[#25D366] to-[#128C7E] px-8 md:px-12 text-sm md:text-base font-bold text-white transition-[transform,box-shadow] duration-300 active:scale-95 hover:scale-105 hover:shadow-[0_10px_40px_rgba(37,211,102,0.4)] will-change-transform"
              >
        ...
        <div
          className={`mt-10 md:mt-12 flex items-center justify-center gap-6 md:gap-12 fade-up delay-2 ${visible ? "visible" : ""}`}
        >
          <a
            href={socialMedia.github}
            target="_blank"
            rel="noopener noreferrer"
            className="group flex flex-col items-center gap-3 md:gap-4 text-[--text-muted] transition-colors hover:text-[--text-primary]"
          >
            {/* Penjelasan: Menghapus filter glass-panel di HP dan membatasi transisi agar hardware acceleration bekerja maksimal. */}
            <div className="flex h-14 w-14 md:h-16 md:w-16 items-center justify-center rounded-full border border-[--border] md:glass-panel bg-[--bg-card] shadow-lg transition-[transform,border-color,box-shadow] duration-300 group-hover:scale-110 group-active:scale-95 group-hover:border-[--border-accent] group-hover:shadow-[0_0_30px_var(--accent-glow)] will-change-transform">
              <svg
                role="img"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
                className="h-6 w-6 md:h-7 md:w-7 fill-[--text-secondary] transition-colors group-hover:fill-[--text-primary]"
              >
